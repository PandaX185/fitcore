...
print((datetime.datetime.now(datetime.timezone.utc)+datetime.timedelta(days=1, hours=1)).isoformat())')"
    req POST /classes 201 "$(python3 -c 'import json,sys
...
req GET /classes 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
if [[ -n "$BR_ID" ]]; then
    req GET "/classes?branchId=$BR_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi

# 4. Fetch by ID; malformed and nil UUIDs are 400, missing is 404.
if [[ -n "$CL_ID" ]]; then
    req GET "/classes/$CL_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi
req GET /classes/not-a-uuid 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/classes/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"

# 5. Patch the class and re-fetch.
if [[ -n "$CL_ID" ]]; then
    req PATCH "/classes/$CL_ID" 200 '{"name":"Spin Advance","capacity":15}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req GET "/classes/$CL_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi
req PATCH "/classes/11111111-1111-1111-1111-111111111111" 404 '{"name":"Nope"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
...
print(datetime.datetime.now(datetime.timezone.utc).isoformat())')"
    req POST /classes 400 "$(python3 -c 'import json,sys,datetime
print(json.dumps({"branchId": sys.argv[1], "name": "Bad Time",
        ...
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req POST /classes 404 '{"branchId":"11111111-1111-1111-1111-111111111111","name":"Ghost","capacity":5,"startsAt":"2027-01-01T10:00:00Z","endsAt":"2027-01-01T11:00:00Z"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
fi

# 7. Delete the class; a second delete is a 404.
if [[ -n "$CL_ID" ]]; then
    req DELETE "/classes/$CL_ID" 204 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    req DELETE "/classes/$CL_ID" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi
