...
req GET /members 200 -n 'list members' "" -H "Authorization: Bearer $ACCESS_TOKEN"

# 3. Create a branch to own the member (members.branch_id is a real FK).
BR_NAME="SmokeMemberBranch-$(date +%s)"
req POST /branches 201 -n 'provision branch for member' "$(python3 -c 'import json,sys
...
req POST /members 201 -n 'create member' "$(python3 -c 'import json,sys
...
if [[ -n "$MB_ID" ]]; then
    req GET "/members/$MB_ID" 200 -n 'fetch member by id' "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi
req GET /members/not-a-uuid 400 -n 'malformed member id rejected' "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/members/00000000-0000-0000-0000-000000000000" 400 -n 'nil member uuid rejected' "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/members/11111111-1111-1111-1111-111111111111" 404 -n 'missing member not found' "" -H "Authorization: Bearer $ACCESS_TOKEN"

# 6. Patch the member (needs members:update) and re-fetch.
if [[ -n "$MB_ID" ]]; then
    req PATCH "/members/$MB_ID" 200 -n 'update member' '{"name":"SmokeMember-updated"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req GET "/members/$MB_ID" 200 -n 'fetch updated member' "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi

# 7. Validation and conflict cases: nil branch → 400, blank name → 400,
#    blank email → 400, @-less email → 400, bad body → 400, duplicate → 409.
req POST /members 400 -n 'nil branch on member rejected' '{"branchId":"00000000-0000-0000-0000-000000000000","name":"Ada","email":"a@b.com"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /members 400 -n 'blank member name rejected' '{"branchId":"00000000-0000-0000-0000-000000000000","name":"","email":"a@b.com"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /members 400 -n 'blank member email rejected' '{"branchId":"00000000-0000-0000-0000-000000000000","name":"Ada","email":""}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /members 400 -n 'invalid member email rejected' '{"branchId":"00000000-0000-0000-0000-000000000000","name":"Ada","email":"nope"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /members 400 -n 'malformed member body rejected' 'not-json' -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /members 409 -n 'duplicate member email rejected' "$(python3 -c 'import json,sys
...
if [[ -n "$MB_ID" ]]; then
    req DELETE "/members/$MB_ID" 204 -n 'delete member' "" -H "Authorization: Bearer $ACCESS_TOKEN"
...
req POST /members 403 -n 'create denied without members:create' '{"branchId":"00000000-0000-0000-0000-000000000000","name":"Forbidden","email":"x@y.z"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req GET /members 403 -n 'list denied without members:read' "" -H "Authorization: Bearer $ACCESS_TOKEN"
