...
req GET /branches 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
...
if [[ -n "$BR_ID" ]]; then
    req GET "/branches/$BR_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi
req GET /branches/not-a-uuid 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/branches/00000000-0000-0000-0000-000000000000" 422 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/branches/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"

# 5. Patch the branch (needs branches:update in the admin grant) and re-fetch.
if [[ -n "$BR_ID" ]]; then
    req PATCH "/branches/$BR_ID" 200 '{"name":"SmokeBranch-updated"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req GET "/branches/$BR_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
fi

# 6. Validation branches: empty name → 422, bad body → 400, out-of-range coord → 422.
req POST /branches 422 '{"name":"","address":"x","latitude":0,"longitude":0}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /branches 400 'not-json' -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /branches 422 '{"name":"Bad Coord","address":"x","latitude":95,"longitude":0}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"

# 7. Negative case — viewer has branches:read only, so create must be 403.
login "$SMOKE_VIEWER" "$SMOKE_VIEWER_PASSWORD"
req POST /branches 403 '{"name":"Forbidden","address":"x","latitude":0,"longitude":0}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
