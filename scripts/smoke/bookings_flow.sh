...
if [[ -n "$CL_ID" && -n "$M1" ]]; then
    req POST /bookings 201 "$(python3 -c 'import json,sys
print(json.dumps({"classId": sys.argv[1], "memberId": sys.argv[2]}))' "$CL_ID" "$M1")" \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    BK_ID=$(printf '%s' "$SMOKE_BODY" | json_get id)

    # 3. Fetch the booking.
    req GET "/bookings/$BK_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"

    # 4. The same member rebooking is a duplicate → 409.
    req POST /bookings 409 "$(python3 -c 'import json,sys
print(json.dumps({"classId": sys.argv[1], "memberId": sys.argv[2]}))' "$CL_ID" "$M1")" \
        -H "Authorization: Bearer $ACCESS_TOKEN"
...
print(json.dumps({"classId": sys.argv[1], "memberId": sys.argv[2]}))' "$CL_ID" "$M2")" \
    ...
    req GET "/classes/$CL_ID/bookings" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"

    # 7. Cancel frees the seat; rebooking succeeds (000005), rebooking while
    #    booked is refused again.
    req POST "/bookings/$BK_ID/cancel" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    req POST /bookings 201 "$(python3 -c 'import json,sys
print(json.dumps({"classId": sys.argv[1], "memberId": sys.argv[2]}))' "$CL_ID" "$M1")" \
        -H "Authorization: Bearer $ACCESS_TOKEN"
...
req GET "/bookings/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/bookings/00000000-0000-0000-0000-000000000000" 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /bookings 400 '{"classId":"11111111-1111-1111-1111-111111111111","memberId":"00000000-0000-0000-0000-000000000000"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
if [[ -n "$CL_ID" ]]; then
    req POST /bookings 404 "$(python3 -c 'import json,sys
