...
if [[ -n "$BR_ID" ]]; then
    req POST /trainers 201 "$(python3 -c 'import json,sys
print(json.dumps({"branchId": sys.argv[1], "name": "Coach Ada",
    ...
    if [[ -n "$TR_ID" ]]; then
        req GET "/trainers/$TR_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
    req GET "/branches/$BR_ID/trainers" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"

    # 4. A second trainer with the same email is refused.
    req POST /trainers 400 "$(python3 -c 'import json,sys
print(json.dumps({"branchId": sys.argv[1], "name": "Duplicate",
    ...
    if [[ -n "$TR_ID" ]]; then
        req PATCH "/trainers/$TR_ID" 200 '{"active": false}' \
            -H "Authorization: Bearer $ACCESS_TOKEN"
        req GET "/trainers/$TR_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
fi
...
req POST /trainers 404 '{"branchId":"11111111-1111-1111-1111-111111111111","name":"Ghost","email":"ghost@smoke.local"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/trainers/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/trainers/00000000-0000-0000-0000-000000000000" 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
