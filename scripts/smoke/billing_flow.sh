...
print((datetime.datetime.now(datetime.timezone.utc)+datetime.timedelta(days=7)).isoformat())')"
    req POST /invoices 201 "$(python3 -c 'import json,sys
print(json.dumps({"memberId": sys.argv[1], "membershipId": sys.argv[2],
    ...
    if [[ -n "$INV_ID" ]]; then
        req GET "/invoices/$INV_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
    req GET "/members/$MB_ID/invoices" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"

    # 4. Mark paid; reverting paid->pending is refused.
    if [[ -n "$INV_ID" ]]; then
        req PATCH "/invoices/$INV_ID" 200 '{"status":"paid"}' \
            -H "Authorization: Bearer $ACCESS_TOKEN"
        req PATCH "/invoices/$INV_ID" 400 '{"status":"pending"}' \
            -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
    req PATCH "/invoices/$INV_ID" 400 '{"status":"garbage"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"

    # 5. Bad currency is refused at issue time.
    req POST /invoices 400 "$(python3 -c 'import json,sys
print(json.dumps({"memberId": sys.argv[1], "membershipId": sys.argv[2],
    ...
    req POST /invoices 404 "$(python3 -c 'import json,sys
...
req GET "/invoices/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/invoices/00000000-0000-0000-0000-000000000000" 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /invoices 400 '{"memberId":"11111111-1111-1111-1111-111111111111","membershipId":"00000000-0000-0000-0000-000000000000","amountCents":100,"currency":"USD","dueAt":"2026-12-01T00:00:00Z"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
