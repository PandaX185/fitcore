...
print(json.dumps({"memberId": sys.argv[1], "packageId": sys.argv[2],
    ...
    req GET "/memberships/$MS_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    req GET "/members/$MB_ID/memberships" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    req POST /memberships 400 "$(python3 -c 'import json,sys
print(json.dumps({"memberId": sys.argv[1], "packageId": sys.argv[2],
                    "branchId": sys.argv[3]}))' "$MB_ID" "$PK_ID" "$BR_ID")" \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    ...
    req PATCH "/memberships/$MS_ID" 200 '{"status":"frozen"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req PATCH "/memberships/$MS_ID" 400 '{"status":"garbage"}' \
        -H "Authorization: Bearer $ACCESS_TOKEN"
    req POST /memberships 201 "$(python3 -c 'import json,sys
print(json.dumps({"memberId": sys.argv[1], "packageId": sys.argv[2],
                    "branchId": sys.argv[3]}))' "$MB_ID" "$PK_ID" "$BR_ID")" \
...
# 6. Missing and malformed lookups.
req GET "/memberships/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/memberships/00000000-0000-0000-0000-000000000000" 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req POST /memberships 400 '{"memberId":"00000000-0000-0000-0000-000000000000","packageId":"00000000-0000-0000-0000-000000000000","branchId":"00000000-0000-0000-0000-000000000000"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
if [[ -n "$BR_ID" ]]; then
    req POST /memberships 404 "$(python3 -c 'import json,sys
