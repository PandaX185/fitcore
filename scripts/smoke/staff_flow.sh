...
if [[ -n "$BR_ID" ]]; then
    req POST /staff 201 "$(python3 -c 'import json,sys
print(json.dumps({"branchId": sys.argv[1], "name": "Desk Agent",
    ...
    if [[ -n "$STF_ID" ]]; then
        req GET "/staff/$STF_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
    req GET "/branches/$BR_ID/staff" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"

    # 4. A second account with the same email is refused.
    req POST /staff 400 "$(python3 -c 'import json,sys
print(json.dumps({"branchId": sys.argv[1], "name": "Duplicate",
    ...
    if [[ -n "$STF_ID" ]]; then
        req PATCH "/staff/$STF_ID" 200 '{"active": false}' \
            -H "Authorization: Bearer $ACCESS_TOKEN"
        req GET "/staff/$STF_ID" 200 "" -H "Authorization: Bearer $ACCESS_TOKEN"
    fi
fi
...
req POST /staff 404 '{"branchId":"11111111-1111-1111-1111-111111111111","name":"Ghost","email":"ghost@smoke.local"}' \
    -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/staff/11111111-1111-1111-1111-111111111111" 404 "" -H "Authorization: Bearer $ACCESS_TOKEN"
req GET "/staff/00000000-0000-0000-0000-000000000000" 400 "" -H "Authorization: Bearer $ACCESS_TOKEN"
